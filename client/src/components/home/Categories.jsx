...
import { useAllCategoriesQuery } from "../../screens/store/services/categoryService";
  ...
  return isFetching ? (
    <div className="flex flex-wrap -mx-4 mb-2">
      {[1, 2, 3, 4, 5, 6].map((item) => (
        <div
          className="w-6/12 sm:w-4/12 md:w-3/12 lg:w-[20%] xl:w-2/12 p-1"
          key={item}
        >
          <Skeleton>
    ...
    data?.categories.length > 0 && (
      ...
      >
        {data.categories.map((category, index) => {
          ...
          return (
            <SwiperSlide
              className="w-full overflow-hidden rounded-lg relative -mt-12"
              key={index}
              virtualIndex={index}
            >
